...
| `scripts/postprocess.py` | Kiểm tra giới hạn schema, tạo trace, metadata và ZIP | `trace.jsonl`, `metadata.json`, `output.zip` |
| `architecture.md` | Mô tả vai trò, quyền đọc/ghi và handoff giữa các agent | Tài liệu kiến trúc ở root repo |
...
- `trace.jsonl` có 50 dòng tương ứng 50 case của lần chạy mới nhất.
- Các field số tiền, số giờ được làm tròn 2 chữ số; những order không có item
  dùng `null` cho `expected_total_brl`, `difference_brl` và `reconciled`.
